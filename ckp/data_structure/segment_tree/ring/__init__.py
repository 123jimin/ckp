...
class ZModRingSegmentTree:
    ...
    def __init__(self, mod: int, init_values: list[int]):
        self._mod = mod
        self._len = n = len(init_values)
        ...
        tree[cap: cap+n] = map(lambda x: x % mod, init_values)
        for i in range(cap-1, 0, -1):
            i2 = i+i
            tree[i] = tree[i2] + tree[i2+1]
    ...
    def _apply(self, node: int, size: int, m, d):
        mod = self._mod
        tree = self._tree
        tree[node] = (tree[node] * m + size * d) % mod

        if node < self._cap:
            self._lazy_mul[node] = (self._lazy_mul[node] * m) % mod
            self._lazy_add[node] = (self._lazy_add[node] * m + d) % mod
    ...
    def sum_range(self, start: int, end: int):
        ...
        start, end = max(start, 0), min(end, self._len)
        if start >= end: return 0
        return self._sum_range(start, end, 1, 0, self._cap) % self._mod
    ...
    def mul_add_to_range(self, start: int, end: int, m, d):
        mod = self._mod
        self._mul_add_to_range(start, end, m % mod, d % mod, 1, 0, self._cap)
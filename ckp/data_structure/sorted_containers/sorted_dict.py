...
class SortedDict(dict):
    ...
    * :func:`SortedList._reset`
    ...
    def __ror__(self, other):
        if not isinstance(other, Mapping): return NotImplemented
        return self.__class__(chain(other.items(), self.items()))
    ...
    def __str__(self):
        items = ', '.join(f"{str(key)}: {str(self[key])}" for key in self._list)
    ...
    def __repr__(self):
        ...
        """
        items = ', '.join(f"{repr(key)}: {repr(self[key])}" for key in self._list)
        return f'SortedDict({{{items}}})'
...
def aheui_push_int_from_trivial(top: int, value: int) -> str:
    ...
    if top%value == 0 and 2 <= (d := top//value) <= 9: return AHEUI_NUMERALS[d] + "나"
...
def _aheui_push_int_get_strategies(func: Callable[[int|None, int, int], str], top: int, value: int):
    ...
    if value%top == 0:
        ...
        yield func(None, value//top) + "따" + func((value//top)*top, value)
        yield "빠" + func(top, value//top) + "따" + func((value//top)*top, value)

        yield func(None, 1+value//top) + "따" + func((1+value//top)*top, value)
        yield "빠" + func(top, 1+value//top) + "따" + func((1+value//top)*top, value)

    if value >= 100:
        value_sqrt = math.isqrt(value)
        for i in range(10):
            target = value_sqrt - i
            if value%target == 0:
                yield func(top, target) + "빠" + func(target, value//target) + "따"
...
aheui_push_int_from.max_optimal = 532

...
from ckp.number_theory.primality_test import is_prime, is_prime_trial_division

class TestPrimeSieve(unittest.TestCase):
    def test_single_alloc(self):
        sieve = PrimeSieve(100000)
        for n in range(10001): self.assertEqual(is_prime_trial_division(n), sieve.is_prime(n), f"primality test for {n=}")

    def test_list_primes(self):
        sieve = PrimeSieve(100000)
        for p in sieve.primes():
            self.assertTrue(sieve.is_prime(p), f"mismatch between primes() and is_prime() for {p=}")
            self.assertTrue(is_prime_trial_division(p), f"{p} is not a prime number")
    ...
    def test_multi_extend(self):
        ...
        for n in range(100001): self.assertEqual(is_prime_trial_division(n), sieve.is_prime(n), f"primality test for {n=}")

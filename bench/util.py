...
def bench(codes:str|list[str], repeats_per_trial=1, num_trials=5, *, global_vars:dict[str]|None = None, log:bool = True) -> list[float]:
    is_list = isinstance(codes, list)
    if not is_list: codes = [codes]
    times: list[list[float]] = [[] for _ in codes]

    for _ in range(num_trials):
        for i, code in enumerate(codes):
            times[i].append(timeit.timeit(code, globals=(global_vars or globals()), number=repeats_per_trial))
